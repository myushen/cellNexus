...
```{r, echo=TRUE, eval=FALSE, out.height = c("139px"), out.width = "120x" }
...
single_cell_counts |> select(cell_id,
                             sample_id,
                             dataset_id,
                             cell_type_unified_ensemble) |>
  zellkonverter::writeH5AD("single_cell_counts.h5ad", 
                           compression = "gzip",
                           verbose = TRUE)
```
...
cellNexus::sample_sce_obj |> metadata() |> purrr::pluck("data") |> arrow::write_parquet(meta_path)
...
get_metadata(local_metadata = meta_path) |>
    filter(file_id_cellNexus_single_cell %in% c("my_sce.h5ad",
                                                file_id_from_cloud)) |>
    get_single_cell_experiment()
