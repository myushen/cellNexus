...
metadata <- get_metadata(cache_directory = "~/scratch/cellNexus/")
metadata
```
...
The metadata includes a series of metacell aggregation levels, beginning with 2, 4, 8, and so on. For example, the value of metacell_2 represents a grouping of cells that can be split into two distinct metacells. Metacells are generated after quality control, which excludes empty droplets, dead cells and doublets.
...
This is helpful if just few genes are of interest (e.g PUM1), as they can be compared across samples. cellNexus accepts ENSEMBL gene ID(s).
...
counts <- metadata |>
  ...
  get_single_cell_experiment(assays = "cpm", features = "ENSG00000206503", cache_directory = "~/scratch/cellNexus") |> 
  suppressMessages() |>
