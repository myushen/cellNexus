...
counts |>
  ...
  # Plot
  ggplot(aes(forcats::fct_reorder(disease, median_count,.desc = TRUE), `HLA.A`,color = file_id_cellNexus_single_cell)) +
  geom_jitter(shape=".") +
...
metadata |> 
  ...
  # Filter and subset
  dplyr::filter(cell_type_unified_ensemble=="nk") |> 
  ...
  # Plot
  tidySingleCellExperiment::join_features("HLA-A", shape = "wide") |> 
  ggplot(aes(tissue_harmonised, `HLA.A`, color = file_id_cellNexus_single_cell)) +
  theme_bw() +
  theme(
