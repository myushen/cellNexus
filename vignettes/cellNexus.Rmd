...
The unified pseudobulk AnnData object was pre-generated outside of this vignette applying quality control and retaining at least 15,000 intersecting genes across samples and hosted on Zenodo to avoid lengthy recompilation. Download the latest version: [pseudobulk_se.h5ad](https://zenodo.org/records/22668580/files/pseudobulk_se.h5ad?download=1). For all versions: [10.5281/zenodo.22668580](https://zenodo.org/records/22668580).
...
Original Census annotations can be retrieved by the function `get_census_metadata()`, and registered to lazy tibble format by [DuckDB](https://duckdb.org/docs/current/clients/r) 
...
Optionally, you can explore CELLxGENE metadata to retrieve additional information, such as the CELLxGENE URL (`explorer_url`), published paper title (`title`), published date (`published at`), and other dataset-level, file-level, or collection-level annotations.

This information can be joined with cellNexus metadata when needed. Note that these additional annotations are not used to produce this vignette.
