...
[`colnames`](https://rdrr.io/r/base/colnames.html)`(``metadata``)`\
...
`metadata`` ``|>`\
`  ``dplyr``::`[`distinct`](https://dplyr.tidyverse.org/reference/distinct.html)`(``dataset_id``, ``cell_type_unified_ensemble``)`` ``|>`\
`  ``dplyr``::`[`count`](https://dplyr.tidyverse.org/reference/count.html)`(``cell_type_unified_ensemble``, sort ``=`` ``TRUE``)`\
`#> # Source:     SQL [?? x 2]`\
`#> # Database:   DuckDB 1.4.3 [unknown@Linux 5.14.0-570.123.1.el9_6.x86_64:R 4.5.3/:memory:]`\
`#> # Ordered by: desc(n)`\
